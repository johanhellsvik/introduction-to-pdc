...
\framesubtitle{on HPC clusters}
 \begin{description}
    ...
    \item [compiled] Cray/Intel/GNU compilers\\ include headers, expand macros (\verb|.i,.ii| )
    \item [assembled] into machine code (\verb|.o, .obj| )
    \item [linked] Static Libraries (\verb|.lib, .a|  ) \\ Shared Library (\verb|.dll, .so| )
    \item ~ 
...
\begin{frame}[fragile]
  ...
  \framesubtitle{specific to \alert{Beskow only}}
